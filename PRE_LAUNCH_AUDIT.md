...
| DIAN (Facturación Electrónica) | 🟡 REQUIERE ACCIÓN DE NEGOCIO | El código es honesto: solo marca una factura como validada si un Proveedor Tecnológico Autorizado confirma la transmisión real; si no, la UI muestra claramente "CUFE DE PRUEBA — NO VALIDADO". **Acción pendiente (no de código):** configurar `DIAN_PROVIDER_URL`/`DIAN_PROVIDER_API_KEY` con un proveedor real (Alegra/Siigo/Facture) antes de que las facturas tengan validez fiscal real ante la DIAN. |
