...
import { PlatformAuthService, PlatformStats, PlatformClinic } from '../../core/services/platform-auth.service';
...
@Component({
    ...
    <div class="platform-dashboard">
      <header>
        <div>
          ...
          <p>{{ auth.admin()?.firstName }} {{ auth.admin()?.lastName }} — visión general de la plataforma</p>
        </div>
        <button class="logout-btn" (click)="auth.logout()">Cerrar sesión</button>
      </header>

      <section class="stats-grid" *ngIf="stats() as s">
        <div class="stat-card">
          ...
          <table>
            ...
            <tbody>
              <tr *ngFor="let c of clinics()">
                ...
                <td>{{ c.businessType === 'independent_vet' ? 'Vet Independiente' : 'Clínica' }}</td>
                <td>{{ c.plan }}</td>
                <td>{{ c.city }}</td>
                <td>{{ c.usersCount }}</td>
  ...
  styles: [`
    ...
    header {
      ...
      p { font-size: 0.85rem; color: #9ca3af; margin: 4px 0 0; }
    }
    .logout-btn {
    ...
    .stat-card {
      ...
      gap: 6px;
      span { font-size: 24px; color: #a78bfa; }
      strong { font-size: 1.8rem; font-weight: 800; }
      small { font-size: 0.78rem; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.4px; }
    }
