  ...
  template: `
    <div class="page-wrapper">
      ...
      <div class="tabs">
        ...
        >
          <span class="material-symbols-outlined">directions_walk</span> Paseos ({{
            bookings().length
          }})
        </button>
      </div>
        ...
        <div>
          ...
          @if (!loading() && walkers().length > 0) {
            <div class="walkers-grid">
              @for (w of walkers(); track w) {
                <div class="walker-card">
                  <div class="walker-avatar">
                    @if (w.photoUrl) {
                      <img [src]="w.photoUrl" [alt]="w.user?.firstName" />
                    }
                    @if (!w.photoUrl) {
                  ...
                  <div class="walker-info">
                    ...
                    @if (w.coverageZones.length > 0) {
                      <div class="coverage-zones">
                        @for (z of w.coverageZones; track z) {
      ...
      @if (activeTab() === 'bookings') {
        <div>
          ...
          @if (!loadingBookings() && filteredBookings().length === 0) {
            <div class="empty-state">
              <span class="material-symbols-outlined empty-icon">directions_walk</span>
      ...
      @if (showNewWalkerForm()) {
        <div class="modal-backdrop" (click)="showNewWalkerForm.set(false)">
          <div class="modal" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body">
              <p class="modal-hint">
                El usuario debe existir con rol <strong>walker</strong>. Ingresa su ID o correo para
                crear su perfil de paseador.
              </p>
              <div class="form-group">
                <label>Correo del usuario walker</label>
                <input
                  type="email"
                  [(ngModel)]="newWalker.userEmail"
                  placeholder="paseador@vetpro.co"
                />
              </div>
              <div class="form-group">
                <label>Bio</label>
                <textarea
                  [(ngModel)]="newWalker.bio"
                  rows="2"
                  placeholder="Descripción del paseador…"
                ></textarea>
              </div>
              <div class="form-row">
                <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input
                  type="text"
                  [(ngModel)]="newWalker.zonesRaw"
                  placeholder="Laureles, El Poblado, Envigado"
                />
              </div>
            </div>
...
export class WalkersListComponent implements OnInit {
  ...
  createWalker() {
    ...
    const payload = {
      ...
      coverageZones: this.newWalker.zonesRaw
        .split(',')
        .map((z) => z.trim())
        .filter(Boolean),
    };
