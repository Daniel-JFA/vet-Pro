...
@Component({
  ...
  template: `
    <div class="page-wrapper">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">
            Hola, {{ auth.currentUser?.firstName }} — aquí están tus paseos asignados
          </p>
        </div>
      </div>
      ...
      <!-- Paseo activo -->
      @if (activeWalk(); as w) {
        <div class="active-walk-card">
          <div class="active-label"><span class="pulse-dot"></span> Paseo en curso</div>
          <div class="active-walk-info">
            <p class="active-address">📍 {{ w.address }}</p>
            <p class="active-time">
              Inicio: {{ w.startedAt | date: 'HH:mm' }} — {{ w.durationMins }} min
            </p>
          </div>
      ...
      @if (!loading()) {
        <div>
          @for (walk of bookings(); track walk) {
            <div class="walk-card" [class]="'status-' + walk.status">
              <div class="walk-time">
              ...
              <div class="walk-body">
                ...
                <p class="walk-duration">
                  ⏱ {{ walk.durationMins }} min ·
                  {{ walk.price | currency: 'COP' : 'symbol-narrow' : '1.0-0' }}
                </p>
                @if (walk.notes) {
              ...
              <div class="walk-status-col">
                ...
                @if (walk.status === 'assigned' || walk.status === 'confirmed') {
                  <button class="btn-start" (click)="startWalk(walk.id)">Iniciar</button>
                }
              </div>
            </div>
          }
  ...
  styles: [
    `
      ...
      .walk-card.status-completed {
        opacity: 0.6;
      }
      .walk-card.status-cancelled {
        opacity: 0.5;
        border-style: dashed;
      }
      .walk-time {
...
export class WalkerScheduleComponent implements OnInit {
  ...
  pendingCount = () =>
    this.bookings().filter((b) => ['assigned', 'confirmed'].includes(b.status)).length;
  completedToday = () => {
    const today = new Date().toDateString();
    return this.bookings().filter(
      (b) => b.status === 'completed' && new Date(b.completedAt!).toDateString() === today,
    ).length;
  };
      ...
      .subscribe({
        ...
        error: () =>
          this.bookings.update((list) =>
            list.map((b) =>
              b.id === id ? { ...b, status: 'walking' as WalkStatus, startedAt: new Date() } : b,
            ),
          ),
      });
  }
      ...
      .subscribe({
        ...
        error: () =>
          this.bookings.update((list) =>
            list.map((b) =>
              b.id === id
                ? { ...b, status: 'completed' as WalkStatus, completedAt: new Date() }
                : b,
            ),
          ),
      });
